...
import { checkGuess, isValidGuess } from '@/lib/games/wordle-data';
...
export function useWordle(solution: string, wordleNumber: number) {
  const [gameState, setGameState] = useState<WordleGameState>({
    ...
    gameStatus: 'playing',
    currentRow: 0,
    solution: '', // Initialize empty, will update via effect
    keyboardState: {},
  });

  // Handle initialization and solution updates
  useEffect(() => {
    if (!solution || !wordleNumber) return;

    const saved = localStorage.getItem(`wordle-${wordleNumber}`);
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        // Ensure we use the latest solution even if loading from valid save
        setGameState({ ...parsed, solution });
      } catch (e) {
