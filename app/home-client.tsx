...
export default function HomeClient({ content, language }: HomeClientProps) {
  const contentData = content || {
    ...
    popular: { title: "", description: "", calculators: [] },
    features: { title: "", description: "", items: [] },
    introduction: { heading: "", title: "", description: "" },
    browse: {
      title: "", description: "", note: "",
      topCategoriesLabel: "", allCalculatorsLabel: "",
      viewAll: "", trustBadge: { title: "", description: "" }
    },
    whyBest: { title: "", items: [] },
    howToUse: { title: "", steps: [], note: "" },
    trustedBy: {
      title: "", description: "", useCases: [], note: "",
      stats: { freeCalculators: "", freeAndSecure: "", availableOnline: "", categories: "" }
    },
    faqs: { title: "", items: [] }
  };

  const fallbackCalculators = [
    { name: "Mortgage Calculator", category: "Financial", description: "Calculate monthly payments and total interest for your home loan", href: "/financial/mortgage-calculator" },
    { name: "BMI Calculator", category: "Health", description: "Calculate your Body Mass Index and understand your weight status", href: "/health/bmi-calculator" },
    { name: "Loan Calculator", category: "Financial", description: "Calculate loan payments and schedules for any type of loan", href: "/financial/loan-calculator" },
    { name: "Percentage Calculator", category: "Maths", description: "Calculate percentages, ratios, and percentage changes easily", href: "/maths/percentage-calculator" },
    { name: "Compound Interest Calculator", category: "Financial", description: "Calculate investment growth over time with compound interest", href: "/financial/compound-interest-calculator" },
    { name: "Calorie Calculator", category: "Health", description: "Calculate daily calorie needs based on your lifestyle and goals", href: "/health/calorie-calculator" }
  ];
  ...
  const calculatorsToUse = contentData.popular.calculators.length > 0
    ? contentData.popular.calculators.map((calc: any, index: number) => {
      ...
      return { ...calc, href: fallbackCalc ? fallbackCalc.href : "#" };
    })
    : fallbackCalculators;

  const categories = [
    {
      id: "financial", name: contentData.categories.financial.name || "Financial",
      description: contentData.categories.financial.description || "Loan, mortgage, investment, and tax calculators",
      icon: categoryIcons.financial, color: "from-green-400 to-green-600",
      bgColor: "bg-green-50", textColor: "text-green-600",
      calculators: getCalculatorCount("financial"), href: getLocalizedCategoryUrl("financial", language),
    },
    {
      id: "health", name: contentData.categories.health.name || "Health & Fitness",
      description: contentData.categories.health.description || "BMI, calorie, nutrition, and medical calculators",
      icon: categoryIcons.health, color: "from-red-400 to-red-600",
      bgColor: "bg-red-50", textColor: "text-red-600",
      calculators: getCalculatorCount("health"), href: getLocalizedCategoryUrl("health", language),
    },
    {
      id: "maths", name: contentData.categories.maths.name || "Maths",
      description: contentData.categories.maths.description || "Algebra, geometry, statistics, and advanced maths",
      icon: categoryIcons.maths, color: "from-blue-400 to-blue-600",
      bgColor: "bg-blue-50", textColor: "text-blue-600",
      calculators: getCalculatorCount("maths"), href: getLocalizedCategoryUrl("maths", language),
    },
    {
      id: "physics", name: contentData.categories.physics.name || "Physics",
      description: contentData.categories.physics.description || "Force, energy, motion, and physics calculations",
      icon: categoryIcons.physics, color: "from-yellow-400 to-yellow-600",
      bgColor: "bg-yellow-50", textColor: "text-yellow-600",
      calculators: getCalculatorCount("physics"), href: getLocalizedCategoryUrl("physics", language),
    },
    {
      id: "construction", name: contentData.categories.construction.name || "Construction",
      description: contentData.categories.construction.description || "Property, rent, and construction calculations",
      icon: categoryIcons.construction, color: "from-purple-400 to-purple-600",
      bgColor: "bg-purple-50", textColor: "text-purple-600",
      calculators: getCalculatorCount("construction"), href: getLocalizedCategoryUrl("construction", language),
    },
    {
      id: "food", name: contentData.categories.food.name || "Food",
      description: contentData.categories.food.description || "Nutrition, calorie, and meal planning calculators",
      icon: categoryIcons.food, color: "from-orange-400 to-red-500",
      bgColor: "bg-orange-50", textColor: "text-orange-600",
      calculators: getCalculatorCount("food"), href: getLocalizedCategoryUrl("food", language),
    },
    {
      id: "sports", name: contentData.categories.sports.name || "Sports",
      description: contentData.categories.sports.description || "Fitness, exercise, and sports-related calculators",
      icon: categoryIcons.sports, color: "from-blue-400 to-cyan-500",
      bgColor: "bg-blue-50", textColor: "text-blue-600",
      calculators: getCalculatorCount("sports"), href: getLocalizedCategoryUrl("sports", language),
    },
    {
      id: "other", name: contentData.categories.other.name || "Other",
      description: contentData.categories.other.description || "Miscellaneous calculators that don't fit into other categories",
      icon: categoryIcons.other, color: "from-gray-400 to-gray-600",
      bgColor: "bg-gray-50", textColor: "text-pink-600",
      calculators: getCalculatorCount("other"), href: getLocalizedCategoryUrl("other", language),
    },
  ];
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8">
            {(contentData.features.items.length > 0 ? contentData.features.items : [
              { title: "Lightning Fast", description: "Get instant results with our optimized calculation engines" },
              { title: "100% Accurate", description: "All calculators are verified by experts and mathematically precise" },
              { title: "Always Free", description: "Complete access to all calculators with no hidden charges ever" }
