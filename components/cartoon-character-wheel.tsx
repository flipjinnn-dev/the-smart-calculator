...
const DEFAULT_CHARACTERS = [
  'Mickey Mouse', 'SpongeBob', 'Bugs Bunny', 'Tom & Jerry', 'Scooby-Doo',
  'Pikachu', 'Elsa', 'Simba', 'Dora', 'Patrick Star',
  'Shrek', 'Woody', 'Buzz Lightyear', 'Nemo', 'Stitch',
  'Goku', 'Naruto', 'Doraemon', 'Hello Kitty', 'Minions',
  'Peppa Pig', 'Bluey', 'Sonic', 'Mario', 'Ash Ketchum'
]
...
export default function CartoonCharacterWheel() {
  ...
  const spinWheel = () => {
    ...
    setTimeout(() => {
      ...
      const winner = characters[winningIndex]
  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-3xl shadow-2xl p-8">
          ...
          >
            <svg viewBox="0 0 500 500" className="w-full h-full" style={{ background: "white", borderRadius: "50%" }}>
              ...
              {characters.map((character, index) => {
                ...
                let fontSize = 24
                if (characters.length > 30) fontSize = 14
                else if (characters.length > 20) fontSize = 16
                else if (characters.length > 12) fontSize = 18
                else if (characters.length > 8) fontSize = 20
                
                if (character.length > 18) fontSize = Math.min(fontSize, 11)
                else if (character.length > 15) fontSize = Math.min(fontSize, 13)
                else if (character.length > 12) fontSize = Math.min(fontSize, 15)
        ...
        {/* Customize Panel */}
        {showCustomize && (
          <div className="mb-6 p-6 bg-white rounded-xl shadow-lg border-2 border-purple-200">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-gray-800">Customize Characters</h3>
              <button
            ...
            <div className="mb-4">
                  ...
                  {characters.map((character, index) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        title={characters.length <= 3 ? "Minimum 3 characters required" : "Remove"}
                      >
                        <X className="w-4 h-4" />
                      </button>
                    </div>
              ...
              <p className="text-xs text-gray-500 mt-2">
                * Minimum 3 characters required to spin the wheel
              </p>
            </div>
          </div>
        )}
        ...
        {result && (
          <div className="mt-8 p-6 bg-gradient-to-r from-purple-100 to-pink-100 rounded-2xl border-2 border-purple-300 animate-bounce-in">
            <div className="text-center">
              <p className="text-lg text-gray-600 mb-2">🎨 Your Character:</p>
